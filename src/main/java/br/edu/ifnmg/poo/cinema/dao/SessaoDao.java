...
public class SessaoDao {
    ...
    public void excluir(Sessao s) { //Faz o papel de excluir uma sessão
        ...
        try {
            ...
            int apagado = pst.executeUpdate(); //Executa o update
            try {
                pst2 = conexao.prepareStatement("delete from ingresso where idsessaopertence=?"); //Deletar a linha da tabela do ingresso que possui o id da sessão (Elimina os ingressos da sessão também)
                pst2.setInt(1, s.getIdsessao()); //Coloca o id da sessão
                pst2.executeUpdate(); //Executa o update
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, e);
            }
            if (apagado > 0) {
